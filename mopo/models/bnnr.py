...
class BNNR:
    ...
    def __init__(self, params):
        ...
        print('[ BNNR ] Initializing model: {} | {} networks | {} elites'.format(params['name'], params['num_networks'], params['num_elites']))
    ...
    def train(self, inputs, targets,
        ...
        print('\n\n[ BNNR ] training start !')
        print('[ BNNR ] Training {} | Holdout: {}'.format(inputs.shape, holdout_inputs.shape))
        with self.sess.as_default():
        ...
        progress.stamp()
        if timer: timer.stamp('bnnr_train')

        self._set_state()
        if timer: timer.stamp('bnnr_set_state')
        ...
        if timer: timer.stamp('bnnr_holdout')

        self._end_train(holdout_losses)
        if timer: timer.stamp('bnnr_end')

        val_loss = (np.sort(holdout_losses)[:self.num_elites]).mean()
        model_metrics = {'val_loss': val_loss}
        print('[ BNNR ] Holdout', np.sort(holdout_losses), model_metrics)
        print('[ BNNR ] Training finish !')
    ...
    def _compile_losses(self, inputs1, inputs2, inc_var_loss=False):
        ...
        """
        mean1, log_var = self._compile_outputs(inputs1, ret_log_var=True)
        mean2, log_var = self._compile_outputs(inputs2, ret_log_var=True)

        tf_ones = tf.ones([1], dtype=tf.float32)

        #print("mean1",mean1)
        #print("mean2",mean2)

        #total_losses = tf.reduce_mean(tf.reduce_mean(tf.log(mean1 + tf.ones(mean1.shape)), axis=-1), axis=-1) - tf.reduce_mean(tf.reduce_mean(tf.log(mean2/(mean2 + tf.ones(mean2.shape))), axis=-1), axis=-1)
        total_losses = tf.reduce_mean(tf.reduce_mean(tf.log(mean1 + tf_ones), axis=-1), axis=-1) - tf.reduce_mean(tf.reduce_mean(tf.log((mean2+1.e-12*tf_ones)/(mean2 + tf_ones)), axis=-1), axis=-1)
        #total_losses = 0.5 * tf.reduce_mean(tf.reduce_mean(mean1*mean1, axis=-1), axis=-1) - tf.reduce_mean(tf.reduce_mean(mean2, axis=-1), axis=-1)
