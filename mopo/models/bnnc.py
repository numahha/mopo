...
class BNNC:
    ...
    def __init__(self, params):
        ...
        print('[ BNNC ] Initializing model: {} | {} networks | {} elites'.format(params['name'], params['num_networks'], params['num_elites']))
        ...
        # Training objects
        self.optimizer = None
        self.sy_train_in, self.sy_train_targ, self.sy_train_w = None, None, None
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        with tf.variable_scope(self.name):
            ...
            self.sy_train_w = tf.placeholder(dtype=tf.float32,
                                                    ...
                                                    name="training_dr_weights")

            if not self.deterministic:
                train_loss = tf.reduce_sum(self._compile_losses(self.sy_train_in, self.sy_train_targ, self.sy_train_w,  inc_var_loss=True))
                train_loss += tf.add_n(self.decays)
                train_loss += 0.01 * tf.reduce_sum(self.max_logvar) - 0.01 * tf.reduce_sum(self.min_logvar)
            else:
                train_loss = self._compile_losses(self.sy_train_in, self.sy_train_targ, self.sy_train_w, inc_var_loss=False)
                train_loss += tf.add_n(self.decays)
            self.mse_loss = self._compile_losses(self.sy_train_in, self.sy_train_targ, self.sy_train_w, inc_var_loss=False)
    ...
    def train(self, inputs, targets, dr_weights,
        ...
        print('\n\n[ BNNC ] training start !')
        print('[ BNNC ] Training {} | Holdout: {}'.format(inputs.shape, holdout_inputs.shape))
        with self.sess.as_default():
        ...
        for epoch in epoch_iter:
            ...
            for batch_num in range(int(np.ceil(idxs.shape[-1] / batch_size))):
                ...
                self.sess.run(
                    self.train_op,
                    feed_dict={self.sy_train_in: inputs[batch_idxs], self.sy_train_targ: targets[batch_idxs], self.sy_train_w: dr_weights[batch_idxs]}
                )
                grad_updates += 1
                ...
                if holdout_ratio < 1e-12:
                    losses = self.sess.run(
                            ...
                            feed_dict={
                                ...
                                self.sy_train_targ: targets[idxs[:, :max_logging]], self.sy_train_w: dr_weights[idxs[:, :max_logging]]
                            }
                        )
                    ...
                    holdout_losses = self.sess.run(
                            ...
                            feed_dict={
                                ...
                                self.sy_train_w: holdout_dr_weights,
                            }
                        )
                    named_losses = [['M{}'.format(i), losses[i]] for i in range(len(losses))]
        ...
        progress.stamp()
        if timer: timer.stamp('bnnc_train')

        self._set_state()
        if timer: timer.stamp('bnnc_set_state')
        ...
        if timer: timer.stamp('bnnc_holdout')

        self._end_train(holdout_losses)
        if timer: timer.stamp('bnnc_end')

        val_loss = (np.sort(holdout_losses)[:self.num_elites]).mean()
        model_metrics = {'val_loss': val_loss}
        print('[ BNNC ] Holdout', np.sort(holdout_losses), model_metrics)
        print('[ BNNC ] Training finish !')
    ...
    def _compile_losses(self, inputs, targets, dr_weights, inc_var_loss=False):
        ...
        #return tf.reduce_mean(tf.reduce_mean( tf.square(mean - targets) * tf.exp(-log_var) + log_var, axis=-1), axis=-1)
        return tf.reduce_mean(tf.reduce_mean( tf.square(mean - targets), axis=-1), axis=-1)
        #return tf.reduce_mean(tf.reduce_mean( tf.abs(mean - targets) * dr_weights, axis=-1), axis=-1)
